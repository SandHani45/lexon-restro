#!/usr/bin/env python3
"""
Lexnorax Print Agent
==================
Lightweight WebSocket bridge from the browser to a local USB/network thermal printer.
No Java. No certificates. No third-party dependencies beyond what Lexnorax already uses.
...
def install_autostart_termux():
    """
    Set up Lexnorax Agent to auto-start on Android via Termux:Boot.
    ...
    script = (
        ...
        "# Starts Lexnorax Print Agent on Termux boot\n"
...
def install_autostart():
    ...
    bat_contents = (
        "@echo off\n"
        ":: Auto-generated by lexnorax_agent.py --install\n"
        ":: Restarts the Lexnorax Print Agent if it ever exits (crash, kill,\n"
    ...
    try:
        ...
        print(f"  Lexnorax Agent will start silently at every Windows login,")
...
def uninstall_autostart():
    ...
    if IS_WINDOWS:
        ...
        if removed_any:
            print("  Auto-start removed. Lexnorax Agent will no longer start at login.")
        else:
...
def print_raw_windows(printer_name: str, data: bytes, retries: int = 3) -> tuple:
    ...
    for attempt in range(retries):
        try:
            ...
            try:
                _win32print.StartDocPrinter(h, 1, ("Lexnorax Receipt", None, "RAW"))
...
async def main():
    ...
    logger.info("=" * 56)
    logger.info("  Lexnorax Print Agent v%s", VERSION)
...
def run_poll_mode(poll_url: str, interval: float = 2.0):  # noqa: C901
    ...
    logger.info("=" * 56)
    logger.info("  Lexnorax Print Agent v%s — POLL MODE", VERSION)
    ...
    while True:
        try:
            ...
            if e.code == 403:
                logger.error("Invalid agent key — check the URL and regenerate key in Lexnorax settings")
                sys.exit(1)
...
def install_autostart_termux_poll(poll_url: str):
    ...
    script = (
        "#!/data/data/com.termux/files/usr/bin/sh\n"
        "# Lexnorax Agent — auto-generated boot script\n"
    ...
    print()
    print("  ✓ Lexnorax Agent will start automatically on boot")
    print(f"  Boot script: {boot_script}")
    print(f"  Logs:        {log_path}")
    print()
...
if __name__ == "__main__":
    ...
    try:
        ...
        logger.info("Lexnorax Print Agent stopped.")
